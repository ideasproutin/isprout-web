...
function V3Recaptcha({
  ...
  return (
    <div className="flex items-center gap-3 py-3 px-4 border border-gray-300 rounded-lg bg-gray-50">
      {/* Checkbox area */}
      <div 
        className="relative flex items-center justify-center w-7 h-7 border-2 border-gray-400 rounded cursor-pointer bg-white"
        onClick={status === 'idle' || status === 'error' ? handleCheckboxClick : undefined}
        style={{ cursor: status === 'idle' || status === 'error' ? 'pointer' : 'default' }}
      >
        {/* Idle - empty checkbox */}
        {status === 'idle' && (
          <div className="w-full h-full hover:bg-gray-50 transition-colors rounded" />
        )}
      ...
      <div className="flex-1">
        ...
        {status === 'error' && (
          <span className="text-sm text-red-600">
            {errorMessage || "Failed"} - <button type="button" onClick={handleReset} className="underline hover:no-underline">Retry</button>
          </span>
        )}
      </div>
