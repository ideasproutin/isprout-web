...
const slides = [
	homePageImages.slideItem1,
	homePageImages.slideItem2,
	homePageImages.slideItem3,
	homePageImages.slideItem4,
	homePageImages.slideItem5,
];
...
function SlideItem({
	...
	const getSlideStyles = () => {
		...
		if (normalizedDiff === 0) {
			return {
				className:
					"opacity-100 scale-100 z-30 left-[50%] -translate-x-1/2 inset-y-0 w-[45%] md:w-[55%] sm:w-[70%]",
				style: {},
			};
		}
		...
		if (normalizedDiff === 1) {
			return {
				className:
					"opacity-60 scale-75 z-20 left-[60%] md:left-[65%] sm:left-[70%] inset-y-[80px] sm:inset-y-[40px] right-0 w-[28%] md:w-[25%] sm:w-[22%]",
				style: {},
			};
		}
		...
		if (normalizedDiff === 2) {
			return {
				className:
					"opacity-40 scale-50 z-10 left-[78%] sm:left-[83%] inset-y-[80px] sm:inset-y-[40px] right-0 w-[18%] sm:w-[13%]",
				style: {},
			};
		}
		...
		if (normalizedDiff === -1) {
			return {
				className:
					"opacity-60 scale-75 z-20 inset-y-[80px] sm:inset-y-[40px] w-[28%] md:w-[25%] sm:w-[22%]",
		...
		if (normalizedDiff === -2) {
			return {
				className:
					"opacity-40 scale-50 z-10 inset-y-[80px] sm:inset-y-[40px] w-[18%] sm:w-[13%]",
...
function SlidesBox({ currentSlide }: { currentSlide: number }) {
	return (
		<div
			className='grow min-h-px min-w-px relative shrink-0 w-full'
...
function Carousel() {
	...
	return (
		<div
			className='flex flex-col gap-6 h-[560px] sm:h-[400px] items-center px-16 md:px-8 sm:px-4 py-10 sm:py-5 relative shrink-0 w-full max-w-[1428px]'
			data-name='Carousel'
		>
...
export default function FutureOfWork() {
	return (
		<div
			className='w-full flex flex-col items-center justify-center py-20 sm:py-12'
			data-name='FutureOfWork'
