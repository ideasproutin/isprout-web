...
const normalizeFieldType = (value: string | undefined) => {
	...
	if (
		...
		return "tel";
	if (normalized === "textarea" || normalized === "multiline")
		return "textarea";
...
const normalizeField = (value: unknown): WebsiteFormField | undefined => {
	...
	const options = asArray(fieldObject.options)
		.map((option) => {
			...
			const label = getString(optionObject, [
				"label",
				"name",
				"title",
				"text",
			]);
			const optionValue = getString(optionObject, [
				"value",
				"id",
				"slug",
				"code",
			...
			return {
				label: label || optionValue || "Option",
	...
	return {
		...
		helperText: getString(fieldObject, [
			"helperText",
			"description",
			"note",
		]),
...
const normalizeConfig = (value: unknown): WebsiteFormConfig | undefined => {
	...
	return {
		formType: getString(configObject, [
			"formType",
			"formKey",
			"code",
			"name",
		]),
		formKey: getString(configObject, [
			"formKey",
			"formType",
			"code",
			"name",
		...
		subtitle: getString(configObject, [
			"subtitle",
			"subTitle",
			"description",
		]),
		description: getString(configObject, ["description", "body"]),
		successMessage: getString(configObject, [
			"successMessage",
			"success_message",
		]),
		submitButtonText: getString(configObject, [
			"submitButtonText",
			"buttonText",
			"submitText",
		]),
...
export const fetchWebsiteForms = async (formType?: string) => {
	...
	const requestPromise = (async () => {
		...
		try {
			const response = await apiClient.get<FormsApiResponse>(
				public_endpoints.getForms,
				{
