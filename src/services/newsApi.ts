		...
		if (Array.isArray(value)) {
			const links = value
				.map((item) => {
					...
					const name = getString(linkObject, [
						"name",
						"title",
						"label",
					]);
...
const normalizeNewsItem = (
	...
	return {
		...
		headImage: getString(item, [
			"head_image",
			"headImage",
			"image",
			"thumbnail",
		]),
		heroImage: getString(item, [
			"hero_image",
			"heroImage",
			"banner",
			"coverImage",
		...
		paragraphs: getStringArray(item, [
			"paragraph",
			"paragraphs",
			"content",
		]),
...
const normalizePagination = (
	...
	const total =
		...
		itemCount;
	const count = getNumber(dataObj || {}, ["count"], itemCount) || itemCount;
...
const normalizeParams = (
	...
	searchText:
		params?.searchText?.trim() || DEFAULT_NEWS_QUERY_PARAMS.searchText,
